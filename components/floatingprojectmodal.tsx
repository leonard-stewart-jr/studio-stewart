...
interface FloatingProjectModalProps {
    open: boolean;
    onClose: () => void;
    src: string;
    width: number;
...
export default function FloatingProjectModal({
    open,
    onClose,
    src,
    width,
    height = 785,
    navOffset = 76 // height of main nav
}: FloatingProjectModalProps) {
    ...
    const [cursorType, setCursorType] = useState<"left" | "right" | "hand">("hand");
    ...
    useEffect(() => {
        ...
        node.classList.add("hide-native-scrollbar");
        return () => {
            if (node) node.classList.remove("hide-native-scrollbar");
        };
    }, []);
    ...
    return (
        <Backdrop
            ...
            $navOffset={navOffset}
        >
            <ModalContentWrap $availableHeight={availableHeight}>
...
// Styles
const Backdrop = styled.div<{ $navOffset: number }>`
  position: fixed;
  left: 0;
  width: 100vw;
  z-index: 1700;
  ...
  padding: 0;
  margin: 0;
  top: ${props => props.$navOffset}px;
  height: calc(100vh - ${props => props.$navOffset}px);
`;

const ModalContentWrap = styled.div<{ $availableHeight: number }>`
  width: 100vw;
  display: flex;
  flex-direction: column;
  align-items: stretch;
  ...
  pointer-events: none;
  background: none;
  min-height: ${props => props.$availableHeight}px;
